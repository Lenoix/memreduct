...
)
{
	...
	if (NT_SUCCESS (status))
	{
		...
		mem_info->physical_memory.total_bytes = basic_info.NumberOfPhysicalPages;
		mem_info->physical_memory.total_bytes *= basic_info.PageSize;

		status = NtQuerySystemInformation (SystemPerformanceInformation, &perf_info, sizeof (perf_info), NULL);

		if (NT_SUCCESS (status))
		{
			mem_info->physical_memory.free_bytes = perf_info.AvailablePages;
			mem_info->physical_memory.free_bytes *= basic_info.PageSize;

			// page file information
			mem_info->page_file.total_bytes = perf_info.TotalCommitLimit;
			mem_info->page_file.total_bytes *= basic_info.PageSize;

			mem_info->page_file.free_bytes = perf_info.AvailablePages;
			mem_info->page_file.free_bytes *= basic_info.PageSize;

			mem_info->page_file.used_bytes = mem_info->page_file.total_bytes - mem_info->page_file.free_bytes;
			mem_info->page_file.percent = _r_calc_percentof64 (mem_info->page_file.used_bytes, mem_info->page_file.total_bytes);
		}

		mem_info->physical_memory.used_bytes = mem_info->physical_memory.total_bytes - mem_info->physical_memory.free_bytes;
